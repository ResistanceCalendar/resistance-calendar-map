...
function Event(properties) {
  ...
  this.props.start_time = properties.event_type=='Group' ? new Date(2100,1,1): moment(properties.start_datetime)._d;
...
function VotingInfoManager(votingInfo) {
...
$(document).on("submit", "form.event-form", function() {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      var events_joined = JSON.parse(Cookies.get('map.bnc.eventsJoined.' + query['email']) || "[]") || [];

      events_joined.push(query['id_obfuscated']);
      Cookies.set('map.bnc.eventsJoined.' + query['email'], events_joined, {expires: 7});
